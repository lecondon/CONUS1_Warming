...
reglist=c(1,2,5,6, 7,10,8,11, 15)
...
storendreg=array(0, dim=c(nreg,4,4))
rarea=rep(0,nreg)
...
for(r in 1:nreg){
  #identify the cells in the region
	...
	for(y in 1:nyear){
	  for(ri in 1:nrun){
	    storendreg[r,y, ri]=mean(storend[rindex,y,ri]) #average ending storage
...
```{r ET_Maps, fig.width=12, height=6.6}
...
plottemp[which(plottemp<minz)]=minz
rastertemp1=rasterFromXYZ(cbind(xUTM, yUTM, plottemp))
plot(rastertemp1, maxpixels=(nx*ny*10), col=rev(brewer.pal(name="Spectral", n=10)), colNA='lightblue',  legend=T, main="Baseline ET (mm)", axes=F, zlim=c(minz, maxz))
...
```{r ET_Lines, fig.width=6, height=12}
#display.brewer.all()
collist=brewer.pal(name='Spectral',n=11)
collist[6]="grey"
display.brewer.pal(name='Spectral',n=11)

ylab="Change in avg. ET [mm] / Change in Temp"

fout=paste(plotdir, "Regional_Differences_PerDT.pdf", sep="")
pdf(fout, width=6, height=10)

for(r in 1:nreg){
	plotdata=c(lhregdif[r,4,1]/1.5, lhregdif[r,4,2]/2, lhregdif[r,4,3]/4)
	if(r==1){
		plot(c(1.5, 2, 4), plotdata, type='b', pch=16, col=collist[r], xlim=c(1,4.5), ylim=c(0,80), xlab="Change in Temp [deg c]",ylab=ylab, lwd=2)
	}
