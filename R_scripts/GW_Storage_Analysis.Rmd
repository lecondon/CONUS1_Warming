...
for(r in 1:nrun){
	...
	for(y in 1:nyear){
	  ...
	  if(r==2){fin=sprintf("/Users/laura/Documents/CONUS/Warming_Runs/Simulation_Averages/Temp%2.1f_Year%d/storage.monthly.12.bin", runlist[r], ylist[y])
	  }else{
	    fin=sprintf("/Users/laura/Documents/CONUS/Warming_Runs/Simulation_Averages/Temp%d_Year%d/storage.monthly.12.bin", runlist[r], ylist[y])
	    #fin="/Users/laura/Documents/CONUS/Warming_Runs/Simulation_Averages/Temp0_Year4/storage.monthly.12.bin"
	  }
		print(fin)
		to.read = file(fin, "rb")
		header=readBin(to.read, integer(), endian = "little", size=4, n=3)
...
for(r in 1:nrun){
	...
	for(y in 4:7){
		...
		if(r==2 & y ==5 ){
			print("Just storage for T1.5 Year5")
		  #storage
...
storendregmm=PRreg=array(0, dim=c(nreg,4,4))
...
for(r in 1:nreg){
  #identify the cells in the region
	...
	temp2=PETB7[rindex]
	temp3=temp2[is.finite(temp2)==T]
	PETreg[r]=mean(temp3, na.rm=T)
	...
	for(y in 1:nyear){
	  for(ri in 1:nrun){
	    ...
	    PRreg[r,y,ri]=mean(precipmm[rindex]-lhmm[rindex,y, ri])
	    if(ri>1){
...
fout=paste(plotdir, "WTD_Ending_4panel2.pdf", sep="")
...
```{r ET_Maps, fig.width=12, fig.height=6.6}
fout=paste(plotdir, "Annual_ET_4panel2.pdf", sep="")
#pdf(fout, width=5.9, height=8.9)
#par(mfrow=c(3,1), mar=c(1,2,3,3))
pdf(fout, width=12, height=6.6)
par(oma=c( 0,0,0,4))
...
Supporting volume clacs for figure 4 disucssion 
```{r store_stats}
...
fout=paste(plotdir, "Ending_Storage_Diff2.pdf", sep="")
pdf(fout, width=12, height=6.6)
par(oma=c( 0,0,0,4))
par(mfrow=c(2,2), mar=c(1,2,3,4))
...
#fout=paste(plotdir, "Daily_StorageTotal_Lines9_19.pdf", sep="")
fout="Daily_StorageTotal_Lines9_19.pdf"
...
lines(1:nday, stordifD[,1]/10^6, lwd=2, col='black')
dev.off()
...
#calculate the range in storage per year for every scenario
temp_sum=array(0,dim=c(4,4,4))
for(y in 1:4){
  start=(y-1)*365+1
  end=y*365
  temp_sum[1,y,]=stordifD[start,]
  temp_sum[2,y,]=apply(stordifD[start:end,],2,max)
  temp_sum[3,y,]=temp_sum[2,y,]-temp_sum[1,y,]
  temp_sum[4,y,]=apply(stordifD[start:end,],2,which.max)
}
#check change in recharge from year 4 to year 1
for(r in 1:4){
  print((temp_sum[3,4,r]-temp_sum[3,1,r])/temp_sum[3,1,r])
}
...
```{r dET_dPET_Scatter}
fout=paste(plotdir, "dET_dPET_Scatter2.pdf", sep="")
...
Regional trends in storage changes
```{r ET_Lines, fig.width=6, fig.height=2}
display.brewer.pal(name='Spectral',n=11)
collist=brewer.pal(name='Spectral',n=11)
collist[6:10]=c('#c7e9b4','#7fcdbb','#41b6c4','#2c7fb8','#253494')
